...
CLI suite), clones nushell's `nu_scripts` completions (with a mirror
...
dot_bashrc                  # bash init: fnm, pixi, mamba, starship, zoxide,
                            # direnv, fzf, KIRO/VSCode agent prompt override
...
- Interactive inits (fnm, pixi completion, micromamba, starship, zoxide,
  ...
  `.zshrc`; zsh adds `compinit`, which its completion system requires. The
  KIRO/VS Code prompt override stays in `.bashrc` only — agent terminals
  spawn bash.

## Toolchain: pixi global

All CLI tools (nu, starship, delta, direnv, fnm, fzf, gh, git, ruff, uv,
helix, zoxide, codex, micromamba, **bash, zsh, bun**, …) are declared in
...
pi-coding-agent, codegraph, claude-mem, omniroute, larksuite, openspec,
agnix…) and installed by `bootstrap.sh` via `bun add -g --trust`.
